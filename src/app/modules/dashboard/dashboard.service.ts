...
const getLoggedInProductSoldItemsFromDB = async (
  period: "daily" | "weakly" | "monthly"
) => {
  const now = new Date();
  const start = new Date(now.getFullYear(), now.getMonth(), now.getDate());
  ...
  switch (period) {
    case "daily":
      match = {
        ...match,
        createdAt: { $gte: start.setDate(start.getDate() - 1) },
      };
      break;
    case "weakly":
      match = {
        ...match,
        createdAt: { $gte: start.setDate(start.getDate() - 7) },
      };
      break;
    case "monthly":
      match = {
        ...match,
        createdAt: { $gte: start.setDate(start.getDate() - 30) },
      };
      break;
  }
...
// Active Users
const totalActiveUserDataInDB = async (
  ...
  const [totalActiveUser, unActiveUser, listTedItems, soldItems] =
    await Promise.all([
...
const getActiveUserAndListedItemAndSoldItemsAndSoldItemsAndCategoryItemsFromDB =
  async (period: "daily" | "weekly" | "monthly") => {
    ...
    // Listed Items with period filter
    const listedItems = await Product.countDocuments({
      status: { $ne: "Draft" },
      createdAt: { $gte: startDate, $lte: now },
    });

    // Sold Items with period filter
    const soldItems = await Product.countDocuments({
      status: "Sold",
      updatedAt: { $gte: startDate, $lte: now },
    });

    // Category count within period
    const category = await Category.countDocuments({
      // @ts-ignore
      createdAt: { $gte: startDate, $lte: now },
    });

    return { totalUser,activeUserObject, listedItems, soldItems, category };
  };

// Trending Categories
const getTrendingCategoriesFromDB = async (
  period: "daily" | "weekly" | "monthly"
) => {
  const now = new Date();
  let startDate: Date;
